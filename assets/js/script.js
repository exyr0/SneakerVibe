...
function plantillaTablaTallasZapatilla(tallas) {
  if (!Array.isArray(tallas) || tallas.length === 0 || typeof tallas[0] !== "object") return "";
  const filas = tallas.map(t => `<tr><td>${t.us}</td><td>${t.uk}</td><td>${t.eu}</td><td>${t.cm}</td></tr>`).join("");
  return `
...
function plantillaSelectTalla(tallas) {
  ...
  if (!tallas || tallas.length === 0) {
    ...
    options = tallas.map(t => `<option value="${t.eu}">EU ${t.eu} (US ${t.us})</option>`).join("");
  } else {
...
function renderProductoPage() {
  ...
  const esZapatilla = Array.isArray(prod.tallas) && prod.tallas[0] && typeof prod.tallas[0] === "object";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch("assets/componentes/navbar.html")
      ...
      .then(html => {
        ...
        navLinks.forEach(link => {
          ...
          const targetPage = new URL(rawHref, window.location.href).pathname.split("/").pop();
  ...
  if (cont1) {
    cont1.appendChild(
      crearCardCategoria("./ropa.html", "./assets/img/ropa-index.png", "Ropa", "Ropa")
    );
  }
  const cont2 = document.getElementById("card-index-2");
  if (cont2) {
    cont2.appendChild(
      crearCardCategoria("./zapatillas.html", "./assets/img/zapatillas-index.png", "Zapatillas", "Zapatillas")
    );
  }
  const cont3 = document.getElementById("card-index-3");
  if (cont3) {
    cont3.appendChild(
      crearCardCategoria("./accesorios.html", "./assets/img/accesorios-index.png", "Accesorios", "Accesorios")
    );
  }

  /* --- Grid de productos (si hay slots) --- */
  const ids = [
    "card-product-1","card-product-2","card-product-3","card-product-4","card-product-5",
    "card-product-6","card-product-7","card-product-8","card-product-9","card-product-10","card-product-11","card-product-12"
  ];
